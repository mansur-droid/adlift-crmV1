...
export function createTwilioMediaTransport({socket,now=()=>Date.now(),bargeInRms=0.035,bargeInFrames=3}={}){
 let streamSid=null;let callSid=null;let lastInboundSequence=0;let speechFrames=0;let speechDetectedAt=null;let outboundSeq=0;let closed=false;const pendingMarks=new Map();
 ...
 return {
  ...
  ingest(evt){const sequence=Number(evt.sequenceNumber||0);if(sequence&&sequence<=lastInboundSequence)return {duplicate:true};if(sequence)lastInboundSequence=sequence;if(evt.event!=='media'||!evt.media?.payload)return {ignored:true};const audio=Buffer.from(evt.media.payload,'base64');const rms=mulawRms(audio);if(rms>=bargeInRms){speechFrames+=1;if(speechFrames>=bargeInFrames&&!speechDetectedAt)speechDetectedAt=now()}else{speechFrames=0;speechDetectedAt=null}return {audio,rms,speechDetected:speechFrames>=bargeInFrames,detectedAtMs:speechDetectedAt,timestampMs:Number(evt.media.timestamp||0),sequence}},
  ...
  clear(){if(!streamSid)return false;pendingMarks.clear();speechFrames=0;speechDetectedAt=null;return send({event:'clear',streamSid})},
