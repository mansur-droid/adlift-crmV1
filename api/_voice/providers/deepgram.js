...
export function createDeepgramSTT({apiKey=process.env.DEEPGRAM_API_KEY,settings={},WebSocketImpl=WebSocket,now=()=>Date.now()}={}){
 ...
 return {
  ...
  connect(nextHandlers={}){handlers=nextHandlers;return new Promise((resolve,reject)=>{
   ...
   openTimer=setTimeout(()=>{try{ws.terminate?.()}catch{};reject(Object.assign(new Error('Deepgram STT connection timeout.'),{code:'STT_TIMEOUT'}))},timeoutMs(settings,'connect_timeout_ms',8000));
   ...
   ws.on('error',e=>{handlers.onError?.(Object.assign(e,{code:e.code||'STT_ERROR'}));if(!opened){clearTimeout(openTimer);reject(e)}});
   ws.on('close',(code,reason)=>{closed=true;handlers.onState?.('closed');handlers.onClose?.({code,reason:String(reason||'')})});
  })},
  sendAudio(buffer){if(!opened||closed||ws?.readyState!==WebSocketImpl.OPEN)return false;lastAudioAt=now();ws.send(buffer);return true},
  finalize(){if(opened&&!closed&&ws?.readyState===WebSocketImpl.OPEN)ws.send(JSON.stringify({type:'Finalize'}))},
  close(){closed=true;clearTimeout(openTimer);if(ws&&ws.readyState===WebSocketImpl.OPEN){try{ws.send(JSON.stringify({type:'CloseStream'}))}catch{};setTimeout(()=>{try{ws.close()}catch{}},20)}else try{ws?.close?.()}catch{}},
