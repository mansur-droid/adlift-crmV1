...
test('Groq compliance validator allows a valid response',async()=>{const fetchImpl=async()=>new Response(JSON.stringify({choices:[{message:{content:JSON.stringify({allowed:true,violations:[],unsupported_claims:[]})}}]}),{status:200,headers:{'content-type':'application/json'}});const llm=createGroqLLM({apiKey:'configured',fetchImpl});const verdict=await llm.validate({candidate:'May I ask how you get new clients?',hardRules:[],knowledge:[],signal:new AbortController().signal});assert.equal(verdict.allowed,true);assert.deepEqual(verdict.violations,[])});
