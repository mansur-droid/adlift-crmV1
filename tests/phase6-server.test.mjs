...
test('missing Deepgram secret blocks before Twilio origination',async()=>{const h=createControlledTestHandler({providerFactory:()=>noCallProvider,requireAdminFn:adminAuth,liveEnabledFn:()=>true,voiceStatusFn:()=>({deepgram:{configured:false},openai:{configured:true}})});const r=res();await h(fakeReq({method:'POST',body:placeBody}),r);assert.equal(r.statusCode,503);assert.equal(r.body.reason,'stt_provider_secret_missing')});
test('missing selected OpenAI secret blocks before Twilio origination',async()=>{const h=createControlledTestHandler({providerFactory:()=>noCallProvider,requireAdminFn:adminAuth,liveEnabledFn:()=>true,voiceStatusFn:()=>({deepgram:{configured:true},openai:{configured:false}})});const r=res();await h(fakeReq({method:'POST',body:placeBody}),r);assert.equal(r.statusCode,503);assert.equal(r.body.reason,'llm_provider_secret_missing')});
