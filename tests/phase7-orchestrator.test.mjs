...
test('interim speech is session state only and not persisted as a final transcript',async()=>{const h=harness();await h.voice.onInterim({text:'partial'});assert.equal(h.rpc.length,0);assert.equal(h.voice.session.interim_transcript,'partial')});

test('one final prospect turn persists once and produces one dynamic agent turn',async()=>{const h=harness();h.voice.pendingFinal='hello';h.voice.pendingFinalStart=10;h.voice.pendingFinalEnd=20;h.voice.pendingConfidence=[0.9];await h.voice.commitProspectTurn('seg-1');assert.equal(h.rpc.filter(x=>x.name==='ai_append_transcript_segment').length,1);assert.equal(h.spoken.length,1);assert.match(h.spoken[0],/Short response/);assert.equal(h.voice.turnNo,1)});

test('barge-in cancels synthesis and clears queued playback',async()=>{const h=harness();await h.voice.generateAgentTurn('hello');assert.ok(h.voice.current);await h.voice.interrupt('test');assert.equal(h.clears,1);assert.equal(h.transportClears,1);assert.equal(h.voice.current,null);assert.equal(h.voice.session.tts_state,'cancelled')});

test('hard validation blocks a draft before synthesis',async()=>{const h=harness({allowed:false});await h.voice.generateAgentTurn('question');assert.equal(h.spoken.length,0);assert.equal(h.inserts.some(x=>x.row?.event_type==='voice.compliance.blocked'),true)});

test('stale generation cannot keep speaking after interruption',async()=>{const h=harness();await h.voice.generateAgentTurn('question');const epoch=h.voice.generationEpoch;await h.voice.interrupt('new speech');assert.ok(h.voice.generationEpoch>epoch);assert.equal(h.voice.current,null)});
